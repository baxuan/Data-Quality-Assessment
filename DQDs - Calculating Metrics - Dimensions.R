...
My_Calculation <- function(MyData, starting_position){
  ...
  write.table(data.frame(
    ...
    starting_position,
    stringsAsFactors = F), 
    file = paste("DQDs - Calculated Metrics - Dimensions - Counts.csv"), sep=",",append = T,row.names = F, col.names = F)
  ...
  write.table(data.frame(
    ...
    starting_position,
    stringsAsFactors = F), 
    file = paste("DQDs - Calculated Metrics - Dimensions - Metrics.csv"), sep=",",append = T,row.names = F, col.names = F)
...
input_file <- "DQDs - Listed relevant attributes - Dimensions - v14.csv"
file_length <- length(count.fields(input_file, sep = "\n"))

library(tidyverse)
current_df <- data.frame(read_csv(input_file, col_names = FALSE))
randomly_ordered_df <- current_df[sample(nrow(current_df)),]  
write.csv(randomly_ordered_df, file="DQDs - Listed relevant attributes - Dimensions.csv", append = FALSE, row.names = FALSE, col.names = FALSE, sep=',')
###

block_count <- file_length%/%my_block
print(paste('Maximum number of possible samples: ',block_count))
...
{
  print(paste("Calculating metrics of Sample number ", i, ", starting from the affiliation block ", sample_index[i]))
  
  AllData <- read.csv(input_file, skip=(sample_index[i]-1)*my_block,
                      nrows = my_block, header=FALSE, sep=",")                      
  ...
  MyData <- as.data.frame(FilteredData2[,c(1,3,4)]) 
...
my_block <- 10000
Dimensions_file <- "DQDs - Listed relevant attributes - Dimensions.csv"
Dimensions_count <- "DQDs - Calculated Metrics - Dimensions - Counts - v14.csv"
Dimensions_count_length <- 40
Dimensions_file_length<-length(count.fields(Dimensions_file, sep = "\n"))

Dimensions_count_data <- read.csv(Dimensions_count, header=FALSE, sep=",")

for (j in 1:Dimensions_count_length)
{  
  Year_sum <- rep(c(0),times=(max_time-min_time+1))
  MyData <- NULL
  
  print(paste("Exporting the Year sum of Sample number ", j, ", starting from the affiliation ", Dimensions_count_data[j,15]))
  AllData <- read.csv(Dimensions_file, skip=(Dimensions_count_data[j,15]-1)*my_block, nrows = my_block, header=FALSE, sep=",")
  FilteredData <- AllData[AllData[,1]>(min_time-1),]
  FilteredData1 <- FilteredData[FilteredData[,1]<(max_time+1),]
  FilteredData2 <- FilteredData1[1:10000,]
  ...
  {
    write.table(data.frame(i,Year_sum[i-min_time+1], stringsAsFactors = F), file="Year_Sum_Dimensions.csv", sep=",",append = T,row.names = F, col.names = F)
  }
}
}
