...
check_formed_datatype_literals <- function(my_time, my_string){ 
  ...
  if (!is.na(my_time))
  {
    a_year <- regmatches(my_time, gregexpr("\\d{4}", my_time))  
...
check_absent_inconsitent_uncondensed_form <- function(my_time, my_string){ 
  ...
  my_time <- regmatches(my_time, gregexpr("\\d{4}", my_time))  
...
My_Calculation <- function(MyData, starting_position){
  ...
  for (j in 1:n_row)
  {
    ...
    else
    {
      ...
      if (data_report$data_existed==0) 
      {
        ...
        Population_presence <- check_population_completeness(my_time, AA_data$country_code, Population_presence) 
  ...
  write.table(data.frame(
    ...
    starting_position,
    stringsAsFactors = F), 
    file = paste("DQDs - Calculated Metrics - ACM - Counts.csv"), sep=",",append = T,row.names = F, col.names = F)
  ...
  write.table(data.frame(
    ...
    starting_position,
    stringsAsFactors = F), 
    file = paste("DQDs - Calculated Metrics - ACM - Metrics.csv"), sep=",",append = T,row.names = F, col.names = F)
  
  print(Population_presence)
  
}


special_string_endings <- c(',','.',' ','\\(','\\)',';','-') # add '-' 2020/06/10
...
my_block <- 10000
my_sample_iteration <- 44

input_file <- "DQDs - Listed relevant attributes - ACM.csv"
...
for (i in 1:39)
{
  print(paste("Sample number ", i, ", starting from the affiliation ", (i-1)*my_block+1))
  
  AllData <- read.csv(input_file, skip=(i-1)*my_block,
                      nrows = my_block, header=FALSE, sep=",")
  
  MyData <- as.data.frame(AllData[,c(2,4)])
  
  My_Calculation(MyData, (i-1)*my_block+1)

}

print(paste("Last Iteration: ", 40))
AllData <- read.csv(input_file, skip=39*my_block,
                    nrows = 10000, header=FALSE, sep=",")
MyData <- as.data.frame(AllData[,c(2,4)])
My_Calculation(MyData,39*my_block+1)


Year_sum <- rep(c(0),times=(max_time-min_time+1))
MyData <- NULL

ACM_file <- "DQDs - Listed relevant attributes - ACM.csv"
AllData <- read.csv(ACM_file, header=FALSE, sep=",")
MyData <- as.data.frame(AllData[,1]) 
for (i in 1:nrow(MyData))
{
  ...
  if (!is.na(a_year))
  {
    is_year <- as.numeric(a_year)
    Year_sum[is_year-min_time+1] <- Year_sum[is_year-min_time+1] + 1
  }else
}
for (i in min_time:max_time)
{
  write.table(data.frame(i,Year_sum[i-min_time+1], stringsAsFactors = F), file="Year_Sum_ACM.csv", sep=",",append = T,row.names = F, col.names = F)
}
